...
class RecitationResultScreen extends StatelessWidget {
  const RecitationResultScreen({super.key});
      ...
      return Scaffold(
        body: Center(
          child: ElevatedButton(
            onPressed: () =>
                Get.until((r) => r.settings.name == AppRoutes.recitationSetup),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            // Return to Home tab instead of staying on Progress tab.
            Get.find<BottomNavController>().currentIndex.value = 0;
            Get.until((r) => r.settings.name == AppRoutes.nav);
          },
        ),
        title: Text(
          'Your Result',
  ...
  Future<void> _nextAyah(RecitationController ctrl) async {
    final result = ctrl.result.value!;
    final evaluatedSurah = result.surahNumber;
    final evaluatedAyah = result.ayahNumber;

    if (evaluatedAyah == 0) {
      // Full Surah mode — advance to next surah, keep Full Surah mode (ayah 0)
      ...
      if (evaluatedAyah < maxAyah) {
        ...
        // End of surah — advance to next surah, first ayah
        final nextSurah = evaluatedSurah < 114 ? evaluatedSurah + 1 : 114;
        ctrl.selectedSurah.value = nextSurah;
        ...
        decoration: BoxDecoration(
          ...
          border: isMissed
              ? Border.all(
                  color: _background,
                  width: 1.4,
                  // Dashed style simulated via a custom painter isn't supported
                  // in plain BoxDecoration; use a slightly different alpha instead.
                )
              : Border.all(color: _background, width: 1.4),
        ),
        child: Text(
          word.word,
          style: TextStyle(
...
class _VoiceFeedbackCardState extends State<_VoiceFeedbackCard> {
  ...
  void initState() {
    super.initState();
    // Auto-play after a short delay so the screen fully renders first
  ...
  void _onTtsDone() {
    ...
    if (has) {
      ...
      // Log each word's ayah data so we can see why _hasWordAudio is false
